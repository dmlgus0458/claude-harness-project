...
`changes/`, `tests/`, `src/` 등 모든 경로에 PROJECT_ROOT를 prefix로 붙인다.
...
## Inputs
ONLY: `changes/[change-name]/tasks.md`
Do not rely on any prior conversation context.

## Process

### Step 0 — Read and Understand
Read `tasks.md` completely. Extract the `## Commands` section and use these exact commands for all build, test, and type-check operations throughout this session.
Read `changes/[change-name]/spec.md` for acceptance criteria.
Read `CLAUDE.md` for coding conventions.
Read only the files listed in `## Design Notes > Affected Files` from `changes/[change-name]/spec.md` to match existing patterns. Do not read beyond this list unless blocked.
...
If build fails, stop and report to orchestrator for build-error-resolver.
...
### Step 3 — Coverage Report (Report Only)
After all implementation tasks, run `[test:coverage]` from tasks.md `## Commands`.
Report coverage per file in your output. Do not block on coverage; tdd-guide handles gaps.

### Step 3.5 — HARD GATE: Run All Verification Targets
...
- [ ] Run [test:coverage]   → already done in Step 3, mark done
...
- Paste the exact error output and stop. Orchestrator will invoke build-error-resolver.
...
Read `changes/[change-name]/spec.md` acceptance criteria.
For each criterion, confirm implementation satisfies it.
Read `changes/[change-name]/tasks.md` and confirm all completed items are checked `[x]`.
...
[Build]: Pass | Fail
[Modified Files]: [list]
[Tasks File]: changes/[change-name]/tasks.md (all checked)
[Next]: Reviewer
```
